    ...
    <div v-if="feedbackOpen" class="modal-overlay">
      <div class="modal-card-glow">
        <div class="modal-card">
          ...
          <textarea id="feedback-text" class="feedback-textarea" v-model="feedbackText" rows="4"
            :disabled="feedbackSubmitting"></textarea>
          <button class="btn btn-press mt-3" :disabled="feedbackSubmitting || !feedbackText.trim()"
            @click="submitFeedback">{{ feedbackSubmitting ? 'Sending…' : 'Submit' }}</button>
        </div>
      </div>
    </div>
  </Transition>
